...
use Drupal\elasticsearch_helper_content\ElasticsearchContentSettingsException;

/**
 * Defines the ElasticsearchIndexSettings entity.
 *
 * @ConfigEntityType(
 *   id = "elasticsearch_index_settings",
 *   label = @Translation("Elasticsearch Index Settings"),
 *   admin_permission = "administer languages",
 *   config_prefix = "esindex_settings",
 *   entity_keys = {
 *     "id" = "id"
 *   },
 ...
 * )
 */
class ElasticsearchContentSettings extends ConfigEntityBase {
   ...
   * @see entity_create()
   */
  public function __construct(array $values, $entity_type = 'elasticsearch_index_settings') {
    if (empty($values['target_entity_type_id'])) {
      throw new ElasticsearchContentSettingsException('Attempt to create content language settings without a target_entity_type_id.');
    }
    if (empty($values['target_bundle'])) {
      throw new ElasticsearchContentSettingsException('Attempt to create content language settings without a target_bundle.');
    }
  ...
  public static function loadByEntityTypeBundle($entity_type_id, $bundle) {
    ...
    $config = \Drupal::entityManager()->getStorage('elasticsearch_index_settings')->load($entity_type_id . '.' . $bundle);
    if ($config == NULL) {
      $config = ElasticsearchContentSettings::create(['target_entity_type_id' => $entity_type_id, 'target_bundle' => $bundle]);
    }
    return $config;
  }
